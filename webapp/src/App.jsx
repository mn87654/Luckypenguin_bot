...
export default function App() {
  ...
  async function checkTask(id) {
    ...
    if (res.ok) {
      ...
      alert('Not verified yet. Join/Open first then press Check.')
    }
  }
  ...
  return (
    <div style={{ padding: 16, maxWidth: 560, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <button onClick={claimDaily} style={btn()}>
          Claim
        </button>
      </div>
        ...
        {tasks.map(t => (
          <div key={t.id} style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 14, opacity: .8 }}>{t.desc}</div>
            </div>
            <button onClick={() => checkTask(t.id)} style={btn()}>
              Check
            </button>
          </div>
        ))}
